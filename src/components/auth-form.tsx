...
export function AuthForm({signedIn=false,updatePassword=false}:{signedIn?:boolean;updatePassword?:boolean}) {
 ...
 return <>{!signedIn&&!updatePassword&&<div className="tabs" aria-label="Account options">{(['login','signup','reset','resend'] as Mode[]).map(item=><button key={item} type="button" disabled={busy} aria-pressed={mode===item} onClick={()=>{setMode(item);setMessage('');}}>{labels[item]}</button>)}</div>}
 <form onSubmit={submit}>{!signedIn&&!updatePassword&&<><label htmlFor="email">Email address</label><input id="email" name="email" type="email" autoComplete="email" required maxLength={254}/></>}
 {!signedIn&&(updatePassword||mode==='login'||mode==='signup')&&<><label htmlFor="password">{updatePassword?'New password':'Password'}</label><input id="password" name="password" type="password" autoComplete={mode==='login'&&!updatePassword?'current-password':'new-password'} minLength={mode==='login'&&!updatePassword?1:12} maxLength={128} required/>{(updatePassword||mode==='signup')&&<p>Use at least 12 characters.</p>}</>}
 <button disabled={busy}>{busy?'Please wait…':signedIn?'Sign out':updatePassword?'Save password':labels[mode]}</button></form>{message&&<p className="message" role="status">{message}</p>}</>;
}
