...
export async function publicSite(){const {data,error}=await db().from('commerce_settings').select('value').eq('key','public_site').single();if(error)throw error;return {saleTerms:defaultSaleTerms,...data.value};}
