...
export function AuthForm({signedIn=false,updatePassword=false}:{signedIn?:boolean;updatePassword?:boolean}) {
 ...
 {!signedIn&&(updatePassword||mode==='login'||mode==='signup')&&<><label htmlFor="password">{updatePassword?'New password':'Password'}</label><input id="password" name="password" type="password" autoComplete={mode==='login'&&!updatePassword?'current-password':'new-password'} minLength={mode==='login'&&!updatePassword?1:8} maxLength={128} required/>{(updatePassword||mode==='signup')&&<p>Use at least 8 characters.</p>}</>}
