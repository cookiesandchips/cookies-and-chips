...
- TaxJar on uses the configured API; off makes no TaxJar calls. Off does not establish zero tax: live checkout needs an explicitly reviewed alternative/no-collection policy or remains unavailable. API failures never silently produce zero tax. Audit toggle changes and revalidate checkout quotes.
